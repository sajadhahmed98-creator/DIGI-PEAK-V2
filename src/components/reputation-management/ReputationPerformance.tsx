...
export function ReputationPerformance() {
  return (
    <section className="py-12 md:py-20 lg:py-24 px-6 relative overflow-hidden bg-black border-y border-white/5">
      {/* Background Gradients - Reputation Theme */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-5xl h-full bg-gradient-to-r from-amber-600/10 via-emerald-500/10 to-indigo-600/10 blur-3xl opacity-50 pointer-events-none" />

      {/* Kinetic Brand Monitoring Waves */}
      <div className="absolute inset-0 z-0 opacity-30 pointer-events-none">
        <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
          <motion.path 
            d="M 0,40 Q 25,65 50,35 T 100,50" 
            fill="none" 
            stroke="#f59e0b" 
            strokeWidth="0.2"
          ...
          />
          <motion.path 
            d="M 0,50 Q 30,25 60,55 T 100,30" 
            fill="none" 
            stroke="#10b981" 
            strokeWidth="0.2"
            initial={{ pathLength: 0, opacity: 0 }}
      ...
      <div className="mx-auto max-w-7xl relative z-10">
        <div className="text-center mb-16">
          ...
          >
            <h2 className="font-heading text-3xl md:text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight mb-4 text-white">
              Reputation <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-emerald-400">Performance.</span>
            </h2>
            <p className="text-muted text-lg max-w-2xl mx-auto">
              Real-world trust rating scaling metrics across our managed enterprise listings.
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
          {[
            { label: "Review Growth", value: 420, prefix: "+", suffix: "%", decimals: 0 },
            { label: "Client Satisfaction", value: 98.4, prefix: "", suffix: "%", decimals: 1 },
            { label: "Average Rating", value: 4.9, prefix: "", suffix: "★", decimals: 1 },
            { label: "Businesses Managed", value: 120, prefix: "", suffix: "+", decimals: 0 },
            { label: "Trust Score Improvement", value: 85, prefix: "+", suffix: "%", decimals: 0 },
            { label: "Brand Monitoring", value: 24, prefix: "", suffix: "/7", decimals: 0 },
          ].map((stat, i) => (
            <motion.div
              key={i}
